...
from netochi.result_processing.result_plotter import plot_results_csv


class EvaluationExporterAndPlotterConfig(BaseModel):
    ...
    def create(self) -> "EvaluationExporterAndPlotter":
        return EvaluationExporterAndPlotter(config=self)


class EvaluationExporterAndPlotter(PipelineConsumer[MappingInput, MappingState[Any, Any], MappingState[Any, Any]]):
    """
    Handles the generation and printing of experiment reports.
    Automatically discovers metrics and formats tables dynamically.
    """

    @icontract.require(lambda config: isinstance(config, EvaluationExporterAndPlotterConfig))
    def __init__(self, config: EvaluationExporterAndPlotterConfig) -> None:
        self.config = config
    ...
    def plot_from_csv(self):
        csv_path = self.pipeline_output.csv_path / f"{self.csv_filename}.csv"
        output_path = self.pipeline_output.plot_path
        plot_results_csv(csv_path=csv_path, output_path=output_path)
