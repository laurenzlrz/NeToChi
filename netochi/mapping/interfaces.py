...
class MappingState(BaseModel, Generic[ANY_MAPPING_INPUT]):
    ...
    def init_random_assignments(self, seed: Optional[int] = None):
...
class HWNetworkMappingState(MappingState[ANY_MAPPING_INPUT], Generic[ANY_MAPPING_INPUT]):
    ...
    def _init_random_hw(self, seed: Optional[int] = None):
...
class BaseMosaicMappingState(HWNetworkMappingState[ANY_MAPPING_INPUT], Generic[ANY_MAPPING_INPUT]):
    ...
    def validate_mosaic_assignments(self) -> 'BaseMosaicMappingState':
        try:
            hw = self.hw
...
class MosaicHWMappingState(BaseMosaicMappingState[ANY_MAPPING_INPUT], HWNetworkMappingState[ANY_MAPPING_INPUT], Generic[ANY_MAPPING_INPUT, PAYLOAD]):
    ...
    @classmethod
    def from_input_and_hw(cls, mapping_input: ANY_MAPPING_INPUT, hw_config: MosaicHardwareConfig) -> 'MosaicHWMappingState[ANY_MAPPING_INPUT, PAYLOAD]':
        N = mapping_input.graph.num_vertices()
        return cls(
            mapping_input=mapping_input,
            hw_config=hw_config,
            neuron_core_idxs_assignment=np.zeros(N, dtype=int),
            neuron_local_idxs_assignment=np.zeros(N, dtype=int),
            neuron_slice_assignments=np.zeros((N, hw_config.max_distance + 1), dtype=int)
        )
