...
class DeltaOptimalSliceAssigner:
    ...
    def __init__(self, hw_config: MosaicHardwareConfig, graph: gt.Graph, cluster_assignment: npt.NDArray[np.int_], local_assignment: npt.NDArray[np.int_]):
        ...
        for tgt in range(graph.num_vertices()):
            tgt_core = int(cluster_assignment[tgt])

            for src in graph.get_in_neighbors(tgt):
                src_core = int(cluster_assignment[src])
        ...
        # 3. create backup
        self.backup_slice_assignment: dict[int, np.ndarray] = {}
        self.backup_connection_counts: dict[int, np.ndarray] = {}
