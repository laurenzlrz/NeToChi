...
class SimAnnealingInferredHWMapper(BaseMapper[MosaicHWMappingState[MappingInput], MappingInput]):
    ...
    @icontract.require(lambda config: isinstance(config, SimAnnealingIHWConfig))
    def __init__(self, config: SimAnnealingIHWConfig) -> None:
        self.config = config
        ...
        total = sum(probs)
        self._normalized_mutation_probs = [p / total for p in probs]
    ...
    def run(self, mapping_input: MappingInput) -> MosaicHWMappingState[MappingInput]:
        ...
        self._state.assignment = best_assignment
    ...
    @icontract.require(lambda self:
                       ...
                       self._opt_slice_assigner is not None)
    def _run_simulated_annealing(self) -> MosaicAssignment:
        ...
        best_assignment = MosaicAssignment(hw=best_hw,
                                           neuron_core_pre_assignment=self._state.c.copy(),
                                           neuron_idx_pre_assignment=self._state.x.copy(),
                                           neuron_slice_assignment=self._opt_slice_assigner.slice_assignment.copy())

        T = self.config.T_start
        while T > self.config.T_min:
            if (time.time() - start_time) > self.config.time_limit:
                if self.config.verbose:
                    self._pipeline_config.print_console(f"Time limit of {self.config.time_limit} seconds reached. Stopping simulated annealing.")
                break
            ...
            for _ in range(steps_per_T):
                ...
                # TODO: Improve the assigner
                # TODO: Check where copying is necessary and where it can be avoided
                self._opt_slice_assigner = mutation.do(self._state)
                ...
                if delta_E < 0 or rndm < math.exp(-delta_E / T):
                    ...
                    if current_energy < best_energy:
                        best_energy = current_energy
                        best_hw = self._state.hw_config
                        best_assignment = MosaicAssignment(
                            hw=best_hw,
                            neuron_core_pre_assignment=self._state.c.copy(),
                            neuron_idx_pre_assignment=self._state.x.copy(),
                            neuron_slice_assignment=self._opt_slice_assigner.slice_assignment.copy()
            ...
            if self.config.verbose:
                self._pipeline_config.print_console(f"Temp: {T:.4f} | Current Energy: {current_energy:.4f} | Best Energy: {best_energy:.2f}")

        return best_assignment

    def _compute_hardware_cost(self, K: int, Nc: int, Nr: int) -> float:
        ...
        num_neurons = self._graph.num_vertices()
    ...
    def _compute_energy(self) -> float:
        ...
        hw_cost = self._compute_hardware_cost(self._state.K, self._state.Nc, self._state.Nr)

        return self.config.weight_inconsistencies * inconsistencies + hw_cost

    def _do_mutation(self) -> Optional[IHWMutation]:
        mutation_cls: IHWMutation = self._rng.choice(self._mutation_types, p=self._normalized_mutation_probs)
